...
for col in ["Total Enrolled","Retention Rate (%)","Student Satisfaction (%)"]:
    fig, ax = plt.subplots()
    sns.barplot(data=comp, x="Term", y=col, hue="Term", legend=False, ax=ax)
    ax.set_title(col); ax.grid(axis="y", linestyle="--", alpha=0.6)
    st.pyplot(fig)
